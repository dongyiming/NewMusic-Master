 ...
 * @data : 2017/6/8 22:49
 */
public class UniqueMusicController extends BaseRecyclerViewController {


    public UniqueMusicController(Context mContext) {
    ...
    public RecyclerView buildRecyclerView(RecyclerView recyclerView) {
        ...
        return new RecyclerViewBuilder(mContext, recyclerView).setGridLayoutManager(2).setItemDecoration(decorationInfo, true).create();
    }

    @Override
    public MultiCommonAdapter setAdapter(final List<MenuInfo> menuList) {
        ...
        MultiCommonAdapter adapter = new MultiCommonAdapter<MenuInfo>(mContext, menuList, new ICommonRecyclerListener<MenuInfo>() {
            ...
            public int setSpanCount(int position, int viewType) {
                return position == menuList.size() - 1 || viewType == 999 ? 2 : 1;
            }
        });
